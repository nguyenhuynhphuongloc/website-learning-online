...
export default function LibraryWritingTest() {
    ...
    const [writingTests, setWritingTests] = useState<{ _id: string; title: string; isCompleted: boolean }[]>([]);
    ...
    const handleClick = (testId: string) => {
        router.push(`/pages/LibraryTestPage/Writing/WritingTestDetail?id=${testId}`);
    };
    ...
    const fetchWritingTests = useCallback(async () => {

        setIsLoading(true);

        try {
            const res = await authFreshToken(
                `http://localhost:8080/writing-test/getALL?page=${page}&limit=${limit}`,
                {
                    method: 'GET',
                    headers: {
                        "Content-Type": "application/json"
                        // Không cần Authorization, authFreshToken sẽ tự thêm
                    }
                }
            );

            if (!res.ok) {
                console.error(`Failed to fetch writing tests: ${res.status}`);
                return;
            }
